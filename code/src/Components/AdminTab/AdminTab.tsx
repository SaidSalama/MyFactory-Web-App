...
const AdminTab = () => {
	...
	//creating a machine
	const createMachines = async ():Promise<boolean> => {
		try {
			const response = await new MachineAPI_Service().create(newMachine);
			//console.log(`response is ${response.status}`);
			if (response.status === 400) //status 400 means a validation error as defined in the machinr controller
			{
				setFormError("Server Error Data not accepted");
				return false;
			}
			return true;
	...
	const handleSubmit = () => {
		if (modalType === "user") {
			...
			//execute an async function to await creating machines and
			// if it it returns true close the modal else it will show th form error
			(async () => {
				const success = await createMachines();
