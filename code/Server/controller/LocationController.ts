...
import LocationRepo from "../repositories/location_repo";

class LocationController {
    public index = async (req: Request, res: Response) => {
        //get the result oh the query
        const reslults = await new LocationRepo().selectAll();
        ...
        res.status(200).json({ status: 200, message: "location", data: reslults });
    };

    public selectOne = async (req: Request, res: Response) => {
        console.log(req.params);
        //get the result oh the query
        const reslults = await new LocationRepo().selectOne(req.params);
