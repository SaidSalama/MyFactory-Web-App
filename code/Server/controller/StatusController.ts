...
import StatusRepo from "../repositories/status_repo";

class StatusController {
    public index = async (req: Request, res: Response) => {
        //get the result oh the query
        const reslults = await new StatusRepo().selectAll();
        ...
        res.status(200).json({ status: 200, message: "Status", data: reslults });
    };

    public selectOne = async (req: Request, res: Response) => {
        console.log(req.params);
        //get the result oh the query
        const reslults = await new StatusRepo().selectOne(req.params);
